...
| En el plugin (fuente de verdad) | Copia gestionada en cada proyecto (`kit.ps1 update`) | Tuyo en cada proyecto (nunca se sobrescribe) |
|---|---|---|
| `com.github.copilot/agents/` — director + 8 agentes | `.github/agents/` | `pipeline.config.ps1` |
...
| `templates/kit.ps1` | `kit.ps1` | `.gitignore`, `.dockerignore` (se fusionan) |
| `scripts/` — init/update, hooks, staging, smoke, prod, estado | (no se copian: `kit.ps1` los llama en el plugin) | |
...
2. Sube la versión en **dos** sitios, con la misma cifra (p. ej. `1.1.0`):
   ...
   - `.github/plugin/marketplace.json` → `"version"` de la entrada del plugin
3. Anota el cambio en `CHANGELOG.md` e indica si hace falta `kit.ps1 update` en los proyectos.
...
En un proyecto de prueba, `$env:KIT_PLUGIN_ROOT = "C:\Users\carr9\Documents\multiagent-kit-copilot\plugins\multiagent-kit"` hace que `kit.ps1` use esa copia directamente. Al terminar, `copilot plugin marketplace remove carlos-kits-copilot --force` y vuelve a añadir el de GitHub.
