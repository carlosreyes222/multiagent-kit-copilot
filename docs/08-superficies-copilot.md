...
| Hooks `.github/hooks/kit.json` (`node kit.js hook …`) | ✔ | ✔ | ✔ |
...
De ahí la regla del kit: **el plugin es la fuente; `node kit.js init` copia a `.github/` lo que VS Code y el cloud agent necesitan; `node kit.js update` lo refresca**.
...
Requisitos: extensión GitHub Copilot Chat, chat en **modo agente**, y haber recargado la ventana tras `node kit.js init`.
...
- **Hooks**: VS Code lee `.github/hooks/kit.json` (`node kit.js hook …`). Los mismos scripts que en la CLI.
...
- Los scripts son Node.js (no PowerShell): `node kit.js …` funciona igual en Windows, macOS, Linux y el cloud agent.
