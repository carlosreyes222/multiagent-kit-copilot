...
3. En `pipeline.config.json`:

```json
"STAGING_PROVIDER": "supabase",
"SUPABASE_DIR": "oanig-backend",
...
"HEALTH_PATH": "/functions/v1/config",
"SMOKE_CMD": "node scripts/smoke.mjs",
"SUB_REPOS": ["oanig-backend"]
```

(`SUPABASE_DIR`: carpeta que contiene `supabase/`, o `"."` si está en la raíz · `SUPABASE_FUNCTIONS`: `[]` = todas las de `supabase/functions` · `SUPABASE_NO_VERIFY_JWT`: igual que "Verify JWT" apagado en el panel · `HEALTH_PATH`: una función pública que responda 200 · `SMOKE_CMD`: opcional, llama a las funciones de la feature contra la variable de entorno `STAGING_URL` · `SUB_REPOS`: si el backend es un git aparte.)
