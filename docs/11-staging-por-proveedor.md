...
```
(el panel se despliega por git — Vercel preview — y se lista en el informe)

**App Android (Kotlin) en emulador — provisional hasta el modo móvil del kit:**
```json
"STAGING_PROVIDER": "comando",
"STAGING_DEPLOY_CMD": "./gradlew installDebug",
"STAGING_URL": "",
"SMOKE_CMD": "adb shell am start -n com.miapp/.MainActivity"
```
(con un emulador o dispositivo conectado; en Windows `.\gradlew installDebug`; sin URL se considera desplegado si el comando termina en 0; el smoke puede ser un flujo Maestro)

**Juego Godot (Android) sin export web:**
```json
"STAGING_PROVIDER": "ninguno"
```
(el release-manager documenta: instalar el .apk exportado en el dispositivo y recorrer la checklist)
