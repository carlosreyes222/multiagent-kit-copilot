...
2. Comprueba que estás en un repositorio git y que `pipeline.config.ps1` existe (si no, indica al usuario que ejecute `/kit-init` y detente).
3. **Detecta el modo**:
   - **PROYECTO NUEVO** si no hay código fuente aparte de los archivos del kit (solo `pipeline.config.ps1`, `AGENTS.md`, `kit.ps1`, `docs/`, `staging/`, `.github/`) o si `BUILD_CMD` y `TEST_CMD` están vacíos y no existe `docs/ARQUITECTURA.md`.
...
5. Registra el inicio con el script (nunca editando el JSON a mano): `pwsh -NoProfile -File kit.ps1 state feature=<slug> type=feature mode=<nuevo|existente> stage=spec`.
...
Delega al **implementador** con spec + ADR (y, en proyecto nuevo, indícale `MODO: PROYECTO NUEVO` para que haga primero el bootstrap: esqueleto, primera prueba que pasa, y rellenar `pipeline.config.ps1` y `AGENTS.md` con los comandos reales). Espera `IMPLEMENTADO: feature/<slug>`.
En proyecto nuevo, tras esta etapa lee `pipeline.config.ps1` y confirma que `BUILD_CMD` y `TEST_CMD` ya no están vacíos; si lo están, devuelve la tarea al implementador.
...
- El comando exacto para promover: `.\kit.ps1 prod`
**Nunca ejecutes `kit.ps1 prod` tú mismo.** La promoción a producción es siempre una acción humana.
...
- Al entrar en cada etapa ejecuta `kit.ps1 state stage=<etapa>` y, al recibir veredictos, `kit.ps1 state qa=APROBADO` / `codigo=…` / `seguridad=…` (claves permitidas en `kit.ps1 state` sin argumentos). Nunca edites `.pipeline/state.json` a mano. `/pipeline continuar <slug>` lee ese estado.
