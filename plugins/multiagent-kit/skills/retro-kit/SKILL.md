...
1. `pipeline.config.ps1`: proveedor de staging, comandos, sub-repos, límites.
2. `.pipeline/state.json` y salida de `pwsh -NoProfile -File kit.ps1 status`.
...
| Staging | proveedor adecuado al proyecto; `kit.ps1 staging`/`smoke` reales, no improvisados; sin mutaciones de datos ni secretos generados | |
...
| Errores del kit citados en informes | mensajes de scripts, hooks, `kit.ps1` | |
...
5. **Cambios propuestos al plugin**, priorizados, cada uno con: archivo del plugin (`agents/x.md`, `scripts/x.ps1`, `skills/x/SKILL.md`, `templates/...`), qué cambiar en una o dos frases, y por qué (evidencia). Máximo 10.
