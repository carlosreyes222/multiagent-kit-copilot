...
## Comandos
Los comandos de instalar / build / test / lint y el proveedor de staging (`STAGING_PROVIDER`) están en `pipeline.config.ps1`. Úsalos desde ahí (`pwsh -NoProfile -File kit.ps1 …`); no inventes otros. Repositorios: `<uno solo | lista $SUB_REPOS>`.
...
- `/deploy-staging` — solo desplegar la rama actual a staging. `/promote-prod` — ver compuertas; la promoción real la hace una persona con `.\kit.ps1 prod`.
...
- Estado del pipeline: solo con `kit.ps1 state clave=valor`; nunca editar `.pipeline/state.json` a mano.
